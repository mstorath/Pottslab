...
addParamValue(ip, 'init', []); % initial guess
...
elseif isa(A, 'linop') % A is a linear operator (function handle of type 'linop')
    ...
    if ~isempty(A.lseSolver)
        solverHandle = str2func(A.lseSolver);
        [w,flag,relres,iter] = feval(solverHandle, M, bCol, par.tol, par.maxit, [], [], par.init(:));
